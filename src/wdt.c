...
{
    int path[8];
    int node = adt_path_offset_trace("/arm-io/wdt", path);
    ...
    if (adt_get_reg(path, "reg", 0, &wdt_base, NULL)) {
    ...
    u32 wdt_version;
    if (ADT_GETPROP(node, "wdt-version", &wdt_version) < 0)
        return;

    if (wdt_version == 2 || wdt_version == 3) {
        u64 wdt_2nd = 0;
        if (adt_get_reg(path, "reg", 2, &wdt_2nd, NULL)) {
