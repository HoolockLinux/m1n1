...
void smc_power_on(void)
{
    ...
    if (adt_path_offset_trace(adt, "/arm-io/smc", smc_path) < 0) {
        printf("SMC: Could not get /arm-io/smc path");
        return;
    }

    if (adt_get_reg(adt, smc_path, "reg", 2, &start, NULL) < 0) {
        printf("SMC: Could not get /arm-io/smc reg");
        return;
    }
